...
void NvidiaH265EncoderImpl::SetRates(
  ...
  if (configuration_.target_bps) {
    ...
    // Reconfigure NVENC with the new bitrate so the encoder actually
    // adapts to network conditions (without this, the initial bitrate
    // is used forever, causing jitter buffer growth on receivers).
    try {
      ...
      // Reconfigure may fail for some parameter combinations — not fatal
    }
  } else {
    configuration_.SetStreamState(false);
  }
